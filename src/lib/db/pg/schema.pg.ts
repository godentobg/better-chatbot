...
// Knowledge Base schema for storing user uploaded files
export const KnowledgeBaseSchema = pgTable("knowledge_base", {
  id: uuid("id").primaryKey().notNull().defaultRandom(),
  name: text("name").notNull(),
  description: text("description"),
  userId: uuid("user_id")
...
export const KnowledgeBaseFileSchema = pgTable("knowledge_base_file", {
  id: uuid("id").primaryKey().notNull().defaultRandom(),
  ...
  content: text("content"), // Extracted text content for searching
  metadata: json("metadata").default({}), // Additional file metadata
  embeddings: json("embeddings"), // For semantic search (optional)
  userId: uuid("user_id")
    .notNull()
    .references(() => UserSchema.id, { onDelete: "cascade" }),
  createdAt: timestamp("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
...
export type KnowledgeBaseEntity = typeof KnowledgeBaseSchema.$inferSelect;
export type KnowledgeBaseFileEntity = typeof KnowledgeBaseFileSchema.$inferSelect;
